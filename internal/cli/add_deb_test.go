...
func TestDEBAddBuildsSignedByHashRepositoryFromExternalPackage(t *testing.T) {
	...
	entity, err := openpgp.NewEntity("SOW CLI APT Test", "", "sow@example.invalid", &packet.Config{Time: func() time.Time { return created }, RSABits: 2048})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSparseAPTMaterializationNeverGeneratesStableTestingComponents(t *testing.T) {
	...
	created := time.Unix(1_500_000_000, 0).UTC()
	entity, err := openpgp.NewEntity("SOW Sparse APT Test", "", "sow@example.invalid", &packet.Config{Time: func() time.Time { return created }, RSABits: 2048})
	if err != nil {
		t.Fatal(err)
	}
