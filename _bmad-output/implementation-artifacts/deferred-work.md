...
- source_spec: `_bmad-output/implementation-artifacts/spec-sow-v2-p0-p1.md`
  summary: Plain create needs a durable ownership and cleanup protocol for stage directories created before its operation journal is persisted.
  evidence: A process termination after stage creation but before `.sow-plain-operation.json` is durable can leave an unjournaled `.sow-plain-stage-*` directory; signing can make that residue contain full RPM copies, so repeated crashes can consume substantial disk space.

## Deferred from: code review of spec-sow-v2-p0-p1.md (2026-08-01)

- The pre-existing RPM reader hashes a retained writable inode, rewinds it, parses the header, and then checks only size and mtime. A same-host non-cooperating writer that can overwrite and restore both bytes and mtime can perform an A-to-B-to-A swap so the recorded SHA-256 comes from A while rendered header facts come from B. P0's final pathname rehash sees restored A and cannot detect that split observation. A future parser-hardening change should parse from an immutable snapshot or rehash and compare the same descriptor after parsing.
