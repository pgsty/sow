...
func (c *Package) dependencies(nevrsTagID, flagsTagID, namesTagID, versionsTagID int) []Dependency {
	// Preserve upstream v1.3.0's legacy parallel-tag API. SOW's production
	// parser reads and validates the raw names/flags/EVR tag lengths directly,
	// so it does not rely on this compatibility helper's NEVRS placeholder.
...
func (c *Package) Size() uint64 {
	if value := c.Header.GetTag(5009).Int64(); value > 0 {
		return uint64(value)
	}
	if value := c.Header.GetTag(1009).Int64(); value > 0 {
		return uint64(value)
	}
	return 0
}
...
func (c *Package) ArchiveSize() uint64 {
	if value := c.Signature.GetTag(271).Int64(); value > 0 {
		return uint64(value)
	}
	if value := c.Signature.GetTag(1007).Int64(); value > 0 {
		return uint64(value)
	}
	if value := c.Header.GetTag(271).Int64(); value > 0 {
		return uint64(value)
	}
	if value := c.Header.GetTag(1046).Int64(); value > 0 {
		return uint64(value)
	}
	return 0
}
